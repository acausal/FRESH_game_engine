...
function readState(condition: Condition, state: GameState): ConditionValue {
  ...
  switch (type) {
    ...
    case 'player_skill': {
      // value = minimum tier required
      if (!target_id) throw new Error('player_skill requires target_id (skill_id)');
      const skill = state.player.skills[target_id];
      if (!skill || !skill.unlocked) return 0;
      return skill.current_tier;
    }
    ...
    case 'quest_state': {
      ...
      const questStatus = state.global.quest_states[target_id];
      // Return completed status as boolean for conditions
      return questStatus?.completed ?? false;
    }
...
export class ConditionEvaluator {
  ...
  getConditionLibrary(): ConditionLibrary {
    return this.library;
  }
